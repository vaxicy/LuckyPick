...
- **选项管理** — 支持 2~10 个选项，可**粘贴多行文本批量导入**（自动按换行 / 逗号 / 顿号拆分），实时检测并标记重复输入
...
- **权重偏好** ⭐ — 给某个选项加权（×2）提高它被选中的概率；骰子模式下加权项掷两次取大
- **排除它再来** 🔄 — 结果页可移除刚获胜的选项后重新抽取，适合逐个淘汰的决策场景
- **撤销删除** ↩️ — 删除历史 / 收藏或清空历史后，可通过 Toast 上的「撤销」一键恢复
- **搜索过滤** 🔍 — 历史记录与收藏夹均支持关键词搜索
- **结果复制** 📋 — 一键把结果文案复制到剪贴板，方便分享
- **首次使用引导** 👋 — 新用户打开时会用 3 步引导介绍核心玩法，可跳过且只出现一次
